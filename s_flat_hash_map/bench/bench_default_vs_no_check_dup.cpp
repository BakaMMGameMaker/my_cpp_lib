...
// ---------------- emplace / emplace_new 策略 ----------------

// UseEmplaceNew = false -> emplace
// UseEmplaceNew = true  -> emplace_new
template <bool UseEmplaceNew, class Map>
inline void bench_emplace(Map &map, const typename Map::key_type &key, std::uint32_t value) {
    if constexpr (UseEmplaceNew) {
        auto it = map.template emplace<mcl::no_check_dup>(key, value);
...
template <bool UseEmplaceNew, class Map, class Key>
static void BM_InsertKeys(benchmark::State &state, KeyDistribution dist, float max_load_factor) {
    ...
    for (auto _ : state) {
        ...
        fill_map_with_keys<UseEmplaceNew>(map, dataset.keys);
        state.PauseTiming();
        counters.add(map);
        state.ResumeTiming();
    }
...
template <bool UseEmplaceNew, class Map, class Key> void register_int_family(const std::string &prefix) {
    ...
    for (float load_factor : kLoadFactors) {
        ...
        auto *insert = benchmark::RegisterBenchmark((prefix + "_insert_" + suffix).c_str(),
                                                    &BM_InsertKeys<UseEmplaceNew, Map, Key>, kDist, load_factor);
...
void register_all_benchmarks() {
    ...
    // unchecked 路径：emplace_new（不检查键是否存在，但我们保证数据集无重复）
    register_int_family<true, flat_map_u32, std::uint32_t>("fmu32_emplace_no_check_dup");
}
