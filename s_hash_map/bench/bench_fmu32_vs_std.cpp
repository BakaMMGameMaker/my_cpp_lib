...
constexpr std::array<UInt32, 3> kSizes = {1 << 10, 1 << 14, 1 << 18};
...
template <typename Key> std::vector<Key> make_random_keys(UInt32 N, std::uint64_t seed = 123456) {
    static_assert(std::is_integral_v<Key>, "random keys require integral types");
    std::mt19937_64 rng(seed);
    std::vector<Key> keys;
    keys.reserve(N);
    for (UInt32 i = 0; i < N; ++i) { keys.push_back(static_cast<Key>(rng())); }
    return keys;
}

template <typename Key> std::vector<Key> make_sequential_keys(UInt32 N) {
    static_assert(std::is_integral_v<Key>, "sequential keys require integral types");
    std::vector<Key> keys;
    keys.reserve(N);
    for (UInt32 i = 0; i < N; ++i) { keys.push_back(static_cast<Key>(i)); }
    return keys;
}

template <typename Key> std::vector<Key> make_cluster_keys(UInt32 N) {
...
template <typename Key> std::vector<Key> make_keys(KeyDistribution dist, UInt32 N) {
    switch (dist) {
    case KeyDistribution::kSequential:
...
template <class Key> IntDataset<Key> prepare_int_dataset(KeyDistribution dist, UInt32 N) {
    IntDataset<Key> data;
...
struct DebugCounters {
    template <class Map> void add(const Map &map) {
        ...
        if constexpr (HasDebugStats<Map>) {
            ...
            if (static_cast<UInt32>(stats.max_probe_len) > max_probe_len_) {
                max_probe_len_ = static_cast<UInt32>(stats.max_probe_len);
            }
...
static void BM_FindMiss_fmu32(benchmark::State &state, KeyDistribution dist, float max_load_factor) {
    ...
    apply_max_load_factor(map, max_load_factor);
    map.reserve(N);
    for (UInt32 i = 0; i < N; ++i) { map.emplace(dataset.keys[i], static_cast<std::uint32_t>(i)); }
